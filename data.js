...
// Content lives in localStorage so edits persist and apply
// instantly across the site in this browser.
// ============================================================

const AR_DATA_KEY = 'ar_site_data';
const AR_CREDS_KEY = 'ar_admin_creds';
const AR_SESSION_KEY = 'ar_admin_session';
...
function arLoadData() {
  try {
    ...
    if (!raw) return JSON.parse(JSON.stringify(AR_DEFAULT_DATA));
    const parsed = JSON.parse(raw);
    return arDeepMerge(AR_DEFAULT_DATA, parsed);
  } catch (e) {
    return JSON.parse(JSON.stringify(AR_DEFAULT_DATA));
  }
}

function arSaveData(data) {
  localStorage.setItem(AR_DATA_KEY, JSON.stringify(data));
}

function arResetData() {
  localStorage.removeItem(AR_DATA_KEY);
}

/* ---------- Storage: admin credentials ---------- */
function arLoadCreds() {
  const raw = localStorage.getItem(AR_CREDS_KEY);
  if (!raw) {
    const def = { username: 'admin', password: 'Admin@123' };
    localStorage.setItem(AR_CREDS_KEY, JSON.stringify(def));
    return def;
  }
...
function arSaveCreds(creds) {
  localStorage.setItem(AR_CREDS_KEY, JSON.stringify(creds));
}
...
/* ---------- Image resize/compress helper (keeps localStorage small) ---------- */
